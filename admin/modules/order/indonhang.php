<?php
	...
	while($row = mysqli_fetch_array($query_order_detail_list)){
		$i++;
		$cell_height = 10; // Chiều cao mặc định cho mỗi dòng
		$product_name = $row['product_name'];
	
		// Xác định chiều cao thực tế dựa trên số dòng của "Tên sản phẩm"
		$name_height = $pdf->GetStringWidth($product_name) > $width_cell[2] ? 2 * $cell_height : $cell_height;
	
		// Lấy chiều cao lớn nhất trong hàng
		$row_height = max($cell_height, $name_height);
	
		// In các cột
		$pdf->Cell($width_cell[0], $row_height, $i, 1, 0, 'C', $fill);
		$pdf->Cell($width_cell[1], $row_height, $row['product_id'], 1, 0, 'C', $fill);
	
		// MultiCell cho Tên sản phẩm
		$x = $pdf->GetX();
		$y = $pdf->GetY();
		$pdf->MultiCell($width_cell[2], $cell_height, $product_name, 1, 'L', $fill);
	
		// Đặt lại vị trí cho cột tiếp theo
		$pdf->SetXY($x + $width_cell[2], $y);
	
		$pdf->Cell($width_cell[3], $row_height, $row['product_quantity'], 1, 0, 'C', $fill);
		$pdf->Cell($width_cell[4], $row_height, number_format($row['product_price'] - ($row['product_price'] / 100 * $row['product_sale'])) . 'đ', 1, 0, 'C', $fill);
		$pdf->Cell($width_cell[5], $row_height, number_format(($row['product_price'] - ($row['product_price'] / 100 * $row['product_sale'])) * $row['product_quantity']) . "đ", 1, 1, 'C', $fill);
    ...
    $pdf->Write(10,'Cửa hàng phụ kiện thú cưng TyRa cảm ơn bạn đã sử dụng dịch vụ.');
	$pdf->Ln(10);
