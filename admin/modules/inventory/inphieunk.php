<?php
	...
	$i = 0;
    $total = 0;
	// Thay đổi đoạn hiển thị tên sản phẩm trong bảng
	while($row = mysqli_fetch_array($query_inventory_detail)){
		$i++;
		$total += $row['product_price_import'] * $row['product_quantity'];
	
		// Tính chiều cao của ô dựa trên số dòng của tên sản phẩm
		$productNameHeight = $pdf->GetStringWidth($row['product_name']) > $width_cell[2] 
							 ? ceil($pdf->GetStringWidth($row['product_name']) / $width_cell[2]) * 10 
							 : 10;
		$rowHeight = max($productNameHeight, 10); // Đảm bảo chiều cao ít nhất là 10
	
		// Vẽ các ô trong hàng, sử dụng chiều cao động
		$pdf->Cell($width_cell[0], $rowHeight, $i, 1, 0, 'C', $fill);
		$pdf->Cell($width_cell[1], $rowHeight, $row['product_id'], 1, 0, 'C', $fill);
	
		// MultiCell cho tên sản phẩm
		$x = $pdf->GetX(); // Lấy vị trí X hiện tại
		$y = $pdf->GetY(); // Lấy vị trí Y hiện tại
		$pdf->MultiCell($width_cell[2], 10, $row['product_name'], 1, 'C', $fill);
		$pdf->SetXY($x + $width_cell[2], $y); // Di chuyển X để tiếp tục các ô khác
	
		$pdf->Cell($width_cell[3], $rowHeight, $row['product_quantity'], 1, 0, 'C', $fill);
		$pdf->Cell($width_cell[4], $rowHeight, number_format($row['product_price_import']).'đ', 1, 0, 'C', $fill);
		$pdf->Cell($width_cell[5], $rowHeight, number_format($row['product_price_import'] * $row['product_quantity']).'đ', 1, 1, 'C', $fill);
    ...
    $pdf->Write(10,'Cửa hàng phụ kiện TYRA cảm ơn bạn đã sử dụng dịch vụ.');
	$pdf->Ln(10);
